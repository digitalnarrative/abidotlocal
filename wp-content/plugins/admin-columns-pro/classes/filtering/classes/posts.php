...
class CAC_Filtering_Model_Posts extends CAC_Filtering_Model {
	...
	public function posts_where_excerpt( $where ) {
		global $wpdb, $filter_values;
		...
		foreach ( $_REQUEST['cpac_filter'] as $name => $value ) {
			...
			switch ( $column->properties->type ) :
				...
				case 'column-taxonomy' :
					...
					// empty or not empty selected
					if ( in_array( $value, array( 'cpac_empty', 'cpac_not_empty' ) ) ) {
						$vars['tax_query'][] = array(
							'taxonomy' => $column->options->taxonomy,
							'terms'    => get_terms( $column->options->taxonomy, array( 'fields' => 'ids'  ) ),
	 ...
	 * @since 1.0
	 */
	public function get_values_by_post_field( $post_field ) {
		global $wpdb;
