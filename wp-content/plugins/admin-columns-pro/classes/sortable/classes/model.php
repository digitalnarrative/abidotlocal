...
abstract class CAC_Sortable_Model {
    ...
    public function get_users_sorted_by_comment_count_clauses( $clauses ) {
		...
		$count_column = "COUNT({$wpdb->comments}.user_id) AS comment_count";

        // @todo: check if this is safe to use with other plugins
		$clauses['fields'] = "{$wpdb->comments}.user_id AS ID, {$count_column}";
		$clauses['where'] = "{$wpdb->comments}.user_id <> 0";

		if ( $this->show_all_results ) {
			$clauses['fields'] = "{$wpdb->users}.ID, {$count_column}";
			$clauses['where'] = '';
			$clauses['join'] = "RIGHT JOIN {$wpdb->users} ON {$wpdb->users}.ID = {$wpdb->comments}.user_id";
		}
    ...
    public function get_users_sorted_by_comment_count( $vars ) {
        ...
        // get users with a comment...
        $comment_query  = new WP_Comment_Query();
        $comment_filter = (object) array(
            'tag' => 'comments_clauses',
            'function' => array( $this, 'get_users_sorted_by_comment_count_clauses'),
        );

        add_filter( $comment_filter->tag, $comment_filter->function );

        $raw_results = $comment_query->query( array(
			'order'  => $vars['order'],
        ) );

        remove_filter( $comment_filter->tag, $comment_filter->function );

        foreach( $raw_results as $raw_result ) {
            $users[ $raw_result->ID ] = $raw_result->comment_count;
        }
