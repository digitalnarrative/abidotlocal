...
class GFQuiz extends GFAddOn {
				...
				'fields' => array(
					array(
						...
						'choices' => array(
							...
							1 => array(
								'label'         => __( 'Instant feedback', 'gravityformsquiz' ),
					...
					array(
						'name'  => 'grading',
						'label' => __( 'Grading', 'gravityformsquiz' ),
						'type'  => 'grading',
					),
				)
			),
	...
	public function add_merge_tags( $form ) {
		...
		foreach ( $quiz_fields as $field ) {
			...
			$merge_tags[] = array( 'group' => $group, 'label' => 'Quiz Results: ' . $field_label, 'tag' => "{quiz:id={$field_id}}" );
			$merge_tags[] = array( 'group' => $group, 'label' => 'Quiz Score: ' . $field_label, 'tag' => "{quiz_score:id={$field_id}}" );
		}
	...
	public function add_quiz_field( $field_groups ) {
		...
		foreach ( $field_groups as &$group ) {
			if ( $group['name'] == 'advanced_fields' ) {
				$group['fields'][] = array( 'class' => 'button', 'value' => __( 'Quiz', 'gravityformsquiz' ), 'onclick' => "StartAddField('quiz');", 'data-type' => 'quiz' );
				break;
			}
		}
		...
		if ( $position == 25 ) {
			...
			<li class="gquiz-setting-choices field_setting">
				...
				<div style="float:right;<?php echo $show_values_style; ?>">
					...
					<label for="gquiz_field_choice_values_visible" class="inline gfield_value_label"><?php _e( 'show values', 'gravityformsquiz' ) ?></label>
				</div>
				<?php _e( 'Quiz Answers', 'gravityformsquiz' ); ?> <?php gform_tooltip( 'gquiz_field_choices' ) ?>
				<br />
