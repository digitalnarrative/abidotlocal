...
class backupbuddy_core {
	...
	public static function loopback_test() {
		...
		if ( is_wp_error( $response ) ) { // Loopback failed. Some kind of error.
			...
			if ( ( $response['body'] == '-1' ) || ( $response['body'] == '0' ) ) { // Loopback succeeded.
				...
				$error = 'Connected to server but unexpected output: `' . htmlentities( $response['body'] . '`. Code: `' . $response['response']['code'] . ' ' . $response['response']['message'] . ' ' . $response['response']['message'] . '`.' );
	 ...
	 * @return	null
	 */
	public static function cleanup_temp_tables( $forceSerial = '' ) {
	...
	public static function validateSchedules() {
		...
		foreach ( pb_backupbuddy::$options['schedules'] as $schedule_id => $schedule ) {
			...
			if ( FALSE === $cron_inverval ) { // Schedule MISSING. Re-schedule.
				$result = backupbuddy_core::schedule_event( $schedule['first_run'], $schedule['interval'], 'pb_backupbuddy-cron_scheduled_backup', array( (int)$schedule_id ) ); // Add new schedule.
				if ( $result === FALSE ) {
			...
			if ( $cron_inverval != $schedule['interval'] ) { // Schedule exists BUT interval is WRONG. Fix it.
				...
				$result = backupbuddy_core::unschedule_event( $cron_run, 'pb_backupbuddy-cron_scheduled_backup', array( (int)$schedule_id ) );; // Delete existing schedule.
				if ( $result === FALSE ) {
				...
				$result = backupbuddy_core::schedule_event( $next_run, $schedule['interval'], 'pb_backupbuddy-cron_scheduled_backup', array( (int)$schedule_id ) ); // Add new schedule.
				if ( $result === FALSE ) {
