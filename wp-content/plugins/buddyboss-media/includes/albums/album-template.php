...
class BuddyBoss_Media_Albums {
	...
	protected function fetch_albums( $args ){
		...
		if( $args['privacy'] !== "'all'" ){
			$WHERE[] = 'a.privacy IN ( '. $args['privacy'] . ')';
		}
		...
		$sql_results = $wpdb->prepare( "SELECT {$columns_all} FROM {$TABLES} WHERE {$WHERE} ORDER BY {$args['orderby']} {$args['order']} {$LIMIT}", $query_placeholders );
		$sql_count = $wpdb->prepare( "SELECT {$columns_count} FROM {$TABLES} WHERE {$WHERE}", $query_placeholders );
...
//		return sprintf( __( '%s photos', 'buddyboss-media' ), $buddyboss_media_albums_template->album->total_items );
		...
		//Temproray fix for release will have to replace with buddyboss_media_albums table later on
		...
		return sprintf( __( '%s photos', 'buddyboss-media' ), $wpdb->get_var($sql) );
...
function buddyboss_media_btn_move_media(){
	...
	if( buddyboss_media_user_can_move_media() ):?>
		...
		<a href="#" class="button bp-secondary-action buddyboss_media_move" onclick="return buddyboss_media_initiate_media_move(this);" data-activity_id="<?php bp_activity_id() ;?>" data-album_id="<?php echo $album_id;?>">
