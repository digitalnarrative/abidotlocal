...
class BBMediaMigration {
	...
	public function bbm_updating_media_table( ) {
		...
		foreach ( $activity_ids as $key => $activity_id ) {
			...
			$image_ids = bp_activity_get_meta( $activity_id ,'buddyboss_media_aid',true );
			...
			if ( ! is_array( $image_ids ) ) {
				...
				$wpdb->insert(
					$wpdb->prefix . 'buddyboss_media', array(
						...
						'upload_date' => $activity_id->post_date,
					),
					array(
						'%d',
						'%d',
			...
			if ( 500 <= $progress_count ) { break; }  //Setting the no of loops run at one go
	...
	public function bbm_fetch_all_photos() {
		...
		global $wpdb;
		$myrows = $wpdb->get_results( $wpdb->prepare( "SELECT activity_id FROM {$wpdb->base_prefix}bp_activity_meta WHERE meta_key=%s ", 'buddyboss_media_aid' ), ARRAY_A );
		$activity_ids = array();
	...
	function bbm_custom_migrate() {
		...
		if ( empty( $status ) || ('INCOMPLETE' == $status)   ) { ?>
			<form action="options-general.php?page=bbm-migration" method="post">
				<p class="submit">
					<input name="bboss_settings_migrate" type="submit" class="button-primary" value="<?php esc_attr_e( 'Migrate Changes' ); ?>" />
...
add_action('init','bbm_table_migrate');